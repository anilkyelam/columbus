...
system clock on lambdas is precise up to nanoseconds. \amirian{I would use less
strong wording here..}While we have no evidence of 
it, we assume that the clocks between different lambdas drift only in the order 
of microseconds. This provides us enough space for synchronization mismatch as we 
sample measurements only on millisecond scales due to noise constraints. 
We believe this to be a reasonable assumption as we did not observe any 
synchronization-related noise in our results.
\amirian{proposed edits}
Assuming that clocks between different lambdas only contain a drift in the order
...
noise in our results, we believe this this a reasonable assumption.
...
\amirian{in this paragraph n and k are lowercase where in the next one they are
upper case...was that intentional?}
