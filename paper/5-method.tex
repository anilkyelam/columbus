...
protocol}
\subsubsection{Broadcasting Collisions}
...
of 10 co-resided instances on each server, the entire  co-residence detection
...
first phase, all the co-resided instances would know one of their neighbors (as 
each phase reveals the ID of the biggest participating instance to others).
\amirian{confused about this sentence}\anil{does that help?} If we use IDs that are 
...
number of co-resided instances (\textit{K}) and decreases the complexity to $O(\log_2N)$, 
