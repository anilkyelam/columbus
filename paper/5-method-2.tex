...
In this section, we present a novel co-residence detector for lambdas,
including: previous solutions to this problem, the unique challenges we faced
with lambdas, how we addressed each of them, and an evalution of the detector
mechanism.
...
\subsection{Design}
\amirian{Alternate title: Design Considerations}
\amirian{each of these sections is basically setting up the problems so that we
can address them later}

\subsubsection{Using Covert Channels for Generality}
\amirian{Alternate title: Generality}
\amirian{I dont think you need to keep say "covert channel"..sounds clunky in
all of these subsubsection titles}
...
present the fastest and perhaps most reliable (in terms of ground
truth\anil{reliable as in trustworthy information}) way for co-residence
detection. In our scenario, each lambda could quickly ``read'' the identifier
...
given that hardware is more homogenous across computing platforms
than software. 
...
\amirian{alternate subtitle: Multiple Party Support}
\amirian{this paragraph differs from the other two in that it doesnt have a
solution for this design consideration}
...
\amirian{needs the high level solution to match the flow of the others}
...
\amirian{alternate subtitle: Broadcasting IDs Quickly or Fast ID Broadcasting}
...
\amirian{I think the problem statement should be a bit clearer}
...
is 1, otherwise it listens for a 0 or 1. If an lambda senses a 1 while
...
\amirian{couldnt this go under the previous subsubsection}
...
\amirian{alternate subtitle: Memory Bus as a Cover Channel}
