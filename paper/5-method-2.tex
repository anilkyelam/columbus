...
\anil{An alternative framing for this section, lets see how you like it. 
Ariana, take a pass and do some 
sign-posting if you like; my brain was no longer concerned with polished text at 
this hour, but try not to make any big changes. If this is bad, we can always go 
back to earlier framing, content that we have at least tried. I will work on section 
6 after I wake up, and we can send it to Stefan. Or you can just send it and say 
that we are still working on section 6.}
...
(through the network or shared remote storage) to quickly identify the 
...
hardware-based covert channels that are, by definition, also accessible to all the 
...
We also assume that lambdas can synchronize themselves to send or listen for each bit 
in time slots. We show both these assumptions to be reasonable in the next section.
As such, we propose a communication protocol that efficiently broadcasts 
just these bit-strings on channel with such properties.
...
(discussed in section~\ref{sec:background:membus}). 
Pseudo-code for the sender instance is shown in Algorithm~\ref{alg:sender}.

\subsubsection{Listening for a bit: Mechanism} 
...
\subsubsection{Listening for a bit: Sampling frequency}
...
\subsubsection{Listening for a bit: Sample Size} 
...
\subsubsection{Listening for a bit: Overcoming noise} 
