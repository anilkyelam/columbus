...
of the cloud providers, we have previously shown in section\todo{cite} that this
covert channel exists on the other clouds, and thus these experiements can be
replicated on their serverless functions as well.  \amirian{arent these two
sentences a repeat from a previous section? Since this isn't pertinent for the
experiment setup, I would remove them (or maybe just keep the sentence about
AWS). If we do want to repeat, feels a bit
out of place}Activating the memory bus covert channel requires address
...
caching mechanism. After execution, AWS caches the lambda containers (\todo{how
long?}) to reuse them\cite{awscontainerreuse} for repeat lambdas and mitigate
...
well. \amirian{a bit lost here}We note that while different lambdas used the
