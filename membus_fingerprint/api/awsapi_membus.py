...
def main():
    ...
    parser.add_argument('-i', '--interval_length', action='store', help='Intervals length')
    ...
    for i in range(args.num_victims):
        req_q.put((args.victim_url, str(args.interval_length)+" "+str(args.num_adv)))

    # Wait a bit and invoke second batch (thrashers)
    if args.attack:
        for i in range(args.num_adv):
            req_q.put((args.adv_url, str(args.num_adv)+" "+str(i)+" "+str(args.interval_length)))
