...
bool write_to_s3(Aws::S3::S3Client const& client, Aws::String const& bucket, Aws::String const& key, Aws::String data) {
...
invocation_response my_handler(invocation_request const& request)
{
   ...
   // Make sure S3 account is provided
   if (success && !return_data) {
      using namespace Aws::Utils::Json;      // another nicer way to parse json i guess (-_-)
      JsonValue json(req.payload);
      if (!json.WasParseSuccessful()) {
         success = false;
         error = "INVALID_BODY_2";
         lprintf("Could not parse request body with AWS json\n");
      }
      ...
      if (success) {
         ...
         if (!v.ValueExists("s3bucket") || !v.ValueExists("s3key") 
            ...
            success = false;
            error = "NO_S3_DETAILS";
            lprintf("Missing input value s3bucket or s3key\n");
         }

         if (success) {
            bucket = v.GetString("s3bucket");
   ...
   try {
      std::ofstream outfile ("/mnt1/efs/one.txt");
      outfile << "my text here!" << std::endl;
      outfile.close();
      std::ifstream infile ("/mnt1/efs/one.txt");
      std::string content( (std::istreambuf_iterator<char>(infile) ), (std::istreambuf_iterator<char>()) );
      lprintf("EFS Output: %s\n", content.c_str());
   }
   catch(std::exception& e){
      success = false;
      error = "INVALID_EFS_MOUNT";
      lprintf("EFS Mount Exception: %s\n", e.what());
   }
...
// Main method from https://aws.amazon.com/blogs/compute/introducing-the-c-lambda-runtime/
int main()
{
    using namespace Aws;
    SDKOptions options;
