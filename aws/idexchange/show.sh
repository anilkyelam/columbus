...
OUT=`echo Exp,Lambda,Region,Phases,Bit-Time,Count,Responded,Clusters,Errors,Size 1, Size 2, Size 3-5, Unaccounted`
...
for f in `ls out/${prefix}*/stats.json`; do
    ...
    # Print all
    LINE=`echo $name, $lambda, $region, $phases, $bitdur, $count, $responded, $clusters, $errors, $size1, $size2, $size3_5, $unaccounted`
    OUT=`echo -e "${OUT}\n${LINE}"`
done
